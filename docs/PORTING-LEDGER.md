...
- **`fetch_url`** — read a vendor status page or a doc, not just screenshot
  it (`take_screenshot` already exists). Needs an SSRF guard and the egress
  allowlist the ROADMAP has under "Next"; that guard is the real work.
